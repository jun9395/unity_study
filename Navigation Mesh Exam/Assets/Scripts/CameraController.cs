...
public class CameraController : MonoBehaviour
{
    ...
    [SerializeField] private float maxDistance = 30;    // 최대 거리
    [SerializeField] private float whellSpeed = 500;    // 마우스 휠 스크롤 속도
    [SerializeField] private float xMoveSpeed = 500;    // 카메라의 y축 회전 속도
    [SerializeField] private float yMoveSpeed = 250;    // 카메라의 x축 회전 속도
    private float yMinLimit = 5;    // 카메라 x축 회전 제한 최소 속도
    private float yMaxLimit = 80;   // 카메라 x축 회전 제한 최대 속도
    private float x, y;             // 마우스 이동 방향값
    ...
    private void Update()
    {
        ...
        distance -= Input.GetAxis("Mouse ScrollWheel") * whellSpeed * Time.deltaTime;

        // 거리는 최소, 최대 거리를 설정해서 그 값을 벗어나지 못하게 한다
